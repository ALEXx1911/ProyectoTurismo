...
function ProvincieCard({imageUrl,name,info,climate,beaches,places,food,festivities}:ProvinceCardProps){
  const playa=false;
  return(
      <div className={classNames("border-2 border-red-400 rounded-md",
          "grid grid-cols-3 grid-rows-3 object-cover overflow-hidden","card"
      )}>
          <img src={imageUrl} alt="xd" className={classNames("w-full object-cover h-full col-start-1 col-end-4 row-start-1 row-end-3",
              "md:col-start-1 md:col-end-4 md:p-3 md:object-cover", "imagenxd"
          )}/>
          <div className={classNames("row-start-3 row-end-4 col-start-1 col-end-4",
              "md:col-start-1 md:col-end-4 md:row-start-3 md:row-end-4 md:mx-2 md:mb-1","text-start")}>
              <h4 className="font-bold text-xl text-center mb-1 titulo">{name}</h4>
              <p>{info}</p>
              <p><span className="font-bold">Clima: </span>{climate}</p>
              <p><span className="font-bold">Lugares Recomendados: </span>{places}</p>
